...
ninehire_bp = Blueprint('ninehire', __name__, url_prefix='/ninehire')
...
@ninehire_bp.route('/')
def index():
    return render_template('ninehire/index.html')

@ninehire_bp.route('/api/applicants')
def get_applicants():
    ...
    while has_more:
        ...
        try:
            ...
            if response.status_code != 200:
                return jsonify({'error': 'Failed to fetch applicants', 'details': response.text}), 500

            data = response.json()
            print(f"Page {page} response: {len(data.get('results', []))} applicants")
    ...
    print(f"Total applicants fetched: {len(all_results)}")
    return jsonify({'results': all_results})

@ninehire_bp.route('/api/save-settings', methods=['POST'])
def save_settings():
    data = request.json
...
@ninehire_bp.route('/api/saved-settings', methods=['GET'])
...
@ninehire_bp.route('/api/load-settings/<int:setting_id>', methods=['POST'])
...
@ninehire_bp.route('/api/delete-settings/<int:setting_id>', methods=['DELETE'])
